...
class TableTest {
    ...
    @Test // we added this test
        //@pre-condition : there is a card in the slot that we want to remove
    void removeCard_SlotIsNotEmpty(){
        fillSomeSlots();
        removeSomeCardsAndAssert();
    }
    //@post-condition : there is no card left in the slot

    @Test // we added this test
        //@pre-condition : the table may contain some cards
    void countCards_AllRemoved(){
        fillAllSlots();
        removeAllCards();
        assertEquals(0,table.countCards());
    }
    //@post-condition : the table contains no cards
